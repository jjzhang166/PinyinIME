...
public class InputModeSwitcher {
	...
	public int switchModeForUserKey(int userKey) {
		...
		Log.d("tfs","userKey-->"+userKey);
		Log.d("tfs","mInputMode-->"+mInputMode);
		if (USERDEF_KEYCODE_LANG_2 == userKey) {//当按下中英文切换键， -2== userkey
			...
			else if (MODE_SKB_SYMBOL2_EN == mInputMode || MODE_SKB_SYMBOL2_CN == mInputMode || MASK_SKB_LAYOUT_SYMBOL2 == mInputMode) {
				newInputMode = MODE_SKB_CHINESE;
			}
		} else if (USERDEF_KEYCODE_SYM_3 == userKey) {//当按下数字键，-3 == userkey ,此时已切换到数字键盘界面
			//
			// 系统键：显示“?123”的键
			/*if (MODE_SKB_CHINESE == mInputMode) {
				newInputMode = MODE_SKB_SYMBOL1_CN;
			...
			newInputMode = MODE_SKB_SYMBOL1_CN;
		} else if (USERDEF_KEYCODE_SHIFT_1 == userKey) {//当按下英文大小写切换键  -1 == userkey
			// shift键：显示“，” 或者 大小写图标的按键。
			if (MODE_SKB_ENGLISH_LOWER == mInputMode) {
				newInputMode = MODE_SKB_ENGLISH_UPPER;
			} else if (MODE_SKB_ENGLISH_UPPER == mInputMode) {
				newInputMode = MODE_SKB_ENGLISH_LOWER;
			}else if ( MASK_SKB_LAYOUT_SYMBOL2 == mInputMode) {
				newInputMode = MODE_SKB_SYMBOL2_EN;
			} else if (MODE_SKB_SYMBOL2_EN == mInputMode) {
				newInputMode = MODE_SKB_SYMBOL2_CN;
			}else if (MODE_SKB_SYMBOL2_CN == mInputMode) {
			...
			//newInputMode = MASK_SKB_LAYOUT_SYMBOL2;
			if (MODE_SKB_CHINESE == mInputMode) {
				newInputMode = MASK_SKB_LAYOUT_SYMBOL2;
			}else if(MODE_SKB_SYMBOL2_EN == mInputMode){
				newInputMode = MASK_SKB_LAYOUT_SYMBOL2;
		...
		Log.d("tfs","newInputMode--->"+newInputMode);
		// 保存新的输入法模式
		saveInputMode(newInputMode);
		// 准备切换输入法状态
	...
	private void prepareToggleStates(boolean needSkb) {
		...
		if (MASK_SKB_LAYOUT_PHONE != layout) {
			...
			// modify by guojian @20161121 for bug 5280
			mToggleStates.mRowIdToEnable = KeyRow.DEFAULT_ROW_ID;
			if (variation == EditorInfo.TYPE_TEXT_VARIATION_EMAIL_ADDRESS) {
				mToggleStates.mRowIdToEnable = mToggleRowEmailAddress;//mToggleRowEmailAddress
			} else if (variation == EditorInfo.TYPE_TEXT_VARIATION_URI) {
				mToggleStates.mRowIdToEnable = mToggleRowUri;//14 mToggleRowUri
			} else if (MASK_LANGUAGE_CN == language) {
				mToggleStates.mRowIdToEnable = mToggleRowCn;//12
			} else if (MASK_LANGUAGE_EN == language) {
				mToggleStates.mRowIdToEnable = mToggleRowEn;//13
			}
			Log.d("guojian","mToggleStates.mRowIdToEnable=====>>>" + mToggleStates.mRowIdToEnable);
		} else {
			if (MASK_CASE_UPPER == charcase) {
